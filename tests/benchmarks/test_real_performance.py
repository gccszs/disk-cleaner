...
def test_real_directory_scan(path: Path, name: str) -> dict:
    ...
    return {
        ...
        'estimation_accuracy': profile.file_count / result.total_count if result.total_count > 0 else 0,
    }
...
def main():
    ...
    if results:
        ...
        print("-" * 65)
        for r in results:
            print(f"{r['name']:<30} {r['file_count']:>10,} {r['scan_time']:>10.3f} {r['throughput']:>15,.0f}")
        ...
        # Save results
        output_file = Path("benchmark_results") / f"d_drive_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
